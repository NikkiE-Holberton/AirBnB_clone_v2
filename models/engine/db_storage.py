...
class DBStorage():
    ...
    def all(self, cls=None):
        instance_dict = {}
        # If no class we go through the classes in the classes in filestorage
        # Find the class, append to instance.id & add it to instance_dict
        if cls is None:
            for class_str, class_actual in DBStorage.bnb_classes.items():
                ...
                for instance in data:
                    instance_dict[
                        class_str +
                        '.' +
                        instance.id] = class_actual.to_dict(
                        instance)
